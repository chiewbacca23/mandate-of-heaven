...
export class PrototypeGame {
    ...
    toggleCardSelection(cardId) {
        console.log('DEBUG: toggleCardSelection called with', cardId, 'type:', typeof cardId);
        console.log('DEBUG: Currently selected cards', this.selectedCards);
        
        // SINGLE SELECT MODE: Clear all previous selections and select only this card
        const wasSelected = this.selectedCards.includes(cardId) || 
                           this.selectedCards.some(id => String(id) === String(cardId));
        
        if (wasSelected) {
            // Deselect if clicking the same card
            this.selectedCards = [];
            console.log('DEBUG: Deselected - now empty');
        } else {
            // Select only this card (clear all others)
            this.selectedCards = [cardId];
            console.log('DEBUG: Selected (single):', cardId);
        }
...
// Make sure PrototypeGame is available globally for onclick handlers
if (typeof window !== 'undefined') {
    window.PrototypeGame = PrototypeGame;
}
